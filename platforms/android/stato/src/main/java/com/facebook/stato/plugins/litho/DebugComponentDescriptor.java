...
import static com.facebook.stato.plugins.inspector.InspectorValue.Type.Enum;
import static com.facebook.stato.plugins.inspector.InspectorValue.Type.Number;
...
import com.facebook.stato.plugins.inspector.HighlightedOverlay;
import com.facebook.stato.plugins.inspector.InspectorValue;
import com.facebook.stato.plugins.inspector.Named;
import com.facebook.stato.plugins.inspector.NodeDescriptor;
import com.facebook.stato.plugins.inspector.Touch;
import com.facebook.stato.plugins.inspector.descriptors.ObjectDescriptor;
...
public class DebugComponentDescriptor extends NodeDescriptor<DebugComponent> {
  ...
  private Map<String, List<Pair<String[], StatoDynamic>>> mOverrides = new HashMap<>();
  private DebugComponent.Overrider mOverrider =
      new DebugComponent.Overrider() {
        ...
        public void applyComponentOverrides(String key, Component component) {
          final List<Pair<String[], StatoDynamic>> overrides = mOverrides.get(key);
          if (overrides == null) {
            return;
          }

          for (Pair<String[], StatoDynamic> override : overrides) {
  ...
  @Override
  public List<Named<StatoObject>> getData(DebugComponent node) throws Exception {
    NodeDescriptor componentDescriptor = descriptorForClass(node.getComponent().getClass());
    if (componentDescriptor.getClass() != ObjectDescriptor.class) {
      return componentDescriptor.getData(node.getComponent());
    }

    final List<Named<StatoObject>> data = new ArrayList<>();

    final StatoObject layoutData = getLayoutData(node);
    if (layoutData != null) {
      data.add(new Named<>("Layout", layoutData));
    }

    final List<Named<StatoObject>> propData = getPropData(node);
    if (propData != null) {
      data.addAll(propData);
    }

    final StatoObject stateData = getStateData(node);
    if (stateData != null) {
  ...
  @Nullable
  private static StatoObject getLayoutData(DebugComponent node) {
  ...
  @Nullable
  private static List<Named<StatoObject>> getPropData(DebugComponent node) throws Exception {
    if (node.canResolve()) {
      return null;
    }
  ...
  @Nullable
  private static StatoObject getStateData(DebugComponent node) throws Exception {
    return DataUtils.getStateData(node, node.getStateContainer());
  }

  @Override
  public void setValue(DebugComponent node, String[] path, StatoDynamic value) {
    List<Pair<String[], StatoDynamic>> overrides = mOverrides.get(node.getGlobalKey());
    if (overrides == null) {
  ...
  @Override
  public StatoObject getExtraInfo(DebugComponent node) {
    StatoObject.Builder extraInfo = new StatoObject.Builder();
  ...
  private static void applyLayoutOverride(
      DebugLayoutNode node, String[] path, StatoDynamic value) {
    switch (path[0]) {
      case "background":
  ...
  private static void applyReflectiveOverride(Object o, String key, StatoDynamic dynamic) {
    try {
