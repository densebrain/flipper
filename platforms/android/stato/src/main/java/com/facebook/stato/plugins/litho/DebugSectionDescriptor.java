...
public class DebugSectionDescriptor extends NodeDescriptor<DebugSection> {
  ...
  @Override
  public List<Named<StatoObject>> getData(DebugSection node) throws Exception {
  ...
  private static @Nullable List<Named<StatoObject>> getPropData(DebugSection node)
      throws Exception {
    final Section section = node.getSection();
    return DataUtils.getPropData(section);
  }

  private static @Nullable StatoObject getStateData(DebugSection node) throws Exception {
    return DataUtils.getStateData(node, node.getStateContainer());
  }

  @Override
  public void setValue(DebugSection node, String[] path, StatoDynamic value) throws Exception {
    // TODO T39526148
  }
