 ...
 * This class is the extension point for the States inspector plugin and is how custom classes and
 * data can be exposed to the inspector.
 */
public abstract class NodeDescriptor<T> {
  @Nullable protected StatesConnection mConnection;
  private DescriptorMapping mDescriptorMapping;

  void setConnection(StatesConnection connection) {
  ...
  /**
   * Invalidate a node. This tells States that this node is no longer valid and its properties
   * and/or children have changed. This will trigger States to re-query this node getting any new
   * data.
   */
        ...
        protected void runOrThrow() throws Exception {
          StatesArray array =
              new StatesArray.Builder()
                  .put(new StatesObject.Builder().put("id", getId(node)).build())
                  .build();
          StatesObject params = new StatesObject.Builder().put("nodes", array).build();
  ...
  /**
   * Invalidate a node in the ax tree. This tells States that this node is no longer valid and its
   * properties and/or children have changed. This will trigger States to re-query this node
   ...
   */
  public abstract List<Named<StatesObject>> getData(T node) throws Exception;

  /** Gets data for AX tree */
  public List<Named<StatesObject>> getAXData(T node) throws Exception {
   ...
   */
  public abstract void setValue(T node, String[] path, StatesDynamic value) throws Exception;
  ...
  /**
   * @return A string indicating how this element should be decorated. Check with the States
   ...
   *     States desktop app to see what values are supported.
   */
   ...
   *     up, etc.
   */
  public StatesObject getExtraInfo(T node) {
    return new StatesObject.Builder().build();
  }
