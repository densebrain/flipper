...
import static com.facebook.states.plugins.inspector.InspectorValue.Type.Enum;
import static com.facebook.states.plugins.inspector.InspectorValue.Type.Number;
...
import com.facebook.states.plugins.inspector.HighlightedOverlay;
import com.facebook.states.plugins.inspector.InspectorValue;
import com.facebook.states.plugins.inspector.Named;
import com.facebook.states.plugins.inspector.NodeDescriptor;
import com.facebook.states.plugins.inspector.Touch;
import com.facebook.states.plugins.inspector.descriptors.ObjectDescriptor;
...
public class DebugComponentDescriptor extends NodeDescriptor<DebugComponent> {
  ...
  private Map<String, List<Pair<String[], StatesDynamic>>> mOverrides = new HashMap<>();
  private DebugComponent.Overrider mOverrider =
      new DebugComponent.Overrider() {
        ...
        public void applyComponentOverrides(String key, Component component) {
          final List<Pair<String[], StatesDynamic>> overrides = mOverrides.get(key);
          if (overrides == null) {
            return;
          }

          for (Pair<String[], StatesDynamic> override : overrides) {
  ...
  @Override
  public List<Named<StatesObject>> getData(DebugComponent node) throws Exception {
    NodeDescriptor componentDescriptor = descriptorForClass(node.getComponent().getClass());
    if (componentDescriptor.getClass() != ObjectDescriptor.class) {
      return componentDescriptor.getData(node.getComponent());
    }

    final List<Named<StatesObject>> data = new ArrayList<>();

    final StatesObject layoutData = getLayoutData(node);
    if (layoutData != null) {
      data.add(new Named<>("Layout", layoutData));
    }

    final List<Named<StatesObject>> propData = getPropData(node);
    if (propData != null) {
      data.addAll(propData);
    }

    final StatesObject stateData = getStateData(node);
    if (stateData != null) {
  ...
  @Nullable
  private static StatesObject getLayoutData(DebugComponent node) {
  ...
  @Nullable
  private static List<Named<StatesObject>> getPropData(DebugComponent node) throws Exception {
    if (node.canResolve()) {
      return null;
    }
  ...
  @Nullable
  private static StatesObject getStateData(DebugComponent node) throws Exception {
    return DataUtils.getStateData(node, node.getStateContainer());
  }

  @Override
  public void setValue(DebugComponent node, String[] path, StatesDynamic value) {
    List<Pair<String[], StatesDynamic>> overrides = mOverrides.get(node.getGlobalKey());
    if (overrides == null) {
  ...
  @Override
  public StatesObject getExtraInfo(DebugComponent node) {
    StatesObject.Builder extraInfo = new StatesObject.Builder();
  ...
  private static void applyLayoutOverride(
      DebugLayoutNode node, String[] path, StatesDynamic value) {
    switch (path[0]) {
      case "background":
  ...
  private static void applyReflectiveOverride(Object o, String key, StatesDynamic dynamic) {
    try {
