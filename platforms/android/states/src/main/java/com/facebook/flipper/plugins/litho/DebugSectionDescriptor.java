...
public class DebugSectionDescriptor extends NodeDescriptor<DebugSection> {
  ...
  @Override
  public List<Named<StatesObject>> getData(DebugSection node) throws Exception {
  ...
  private static @Nullable List<Named<StatesObject>> getPropData(DebugSection node)
      throws Exception {
    final Section section = node.getSection();
    return DataUtils.getPropData(section);
  }

  private static @Nullable StatesObject getStateData(DebugSection node) throws Exception {
    return DataUtils.getStateData(node, node.getStateContainer());
  }

  @Override
  public void setValue(DebugSection node, String[] path, StatesDynamic value) throws Exception {
    // TODO T39526148
  }
