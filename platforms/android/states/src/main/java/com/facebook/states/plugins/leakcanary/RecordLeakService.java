...
/**
 * When a leak is detected, sends results to connected States desktop app. In order to use this
...
public class RecordLeakService extends AbstractAnalysisResultService {
  ...
  protected void onHeapAnalyzed(HeapDump heapDump, AnalysisResult result) {
    ...
    if (client != null) {
      final LeakCanaryStatesPlugin plugin = client.getPlugin("LeakCanary");
      if (plugin != null) {
