...
public class ApplicationDescriptor extends NodeDescriptor<ApplicationWrapper> {
  ...
  private void setDelegates(ApplicationWrapper node) {
    ...
    for (View view : node.getViewRoots()) {
      ...
      if (view instanceof ViewGroup && !hasDelegateAlready) {
        ...
        // add delegate to root to catch accessibility events so we can update focus in States
              ...
              public boolean onRequestSendAccessibilityEvent(
                ...
                if (mConnection != null) {
                  ...
                  if (eventType == AccessibilityEvent.TYPE_VIEW_ACCESSIBILITY_FOCUSED) {
                    mConnection.send(
                        "axFocusEvent", new StatesObject.Builder().put("isFocus", true).build());
                  } else if (eventType
                      == AccessibilityEvent.TYPE_VIEW_ACCESSIBILITY_FOCUS_CLEARED) {
                    mConnection.send(
                        "axFocusEvent", new StatesObject.Builder().put("isFocus", false).build());
  ...
  @Override
  public List<Named<StatesObject>> getData(ApplicationWrapper node) {
    return Collections.EMPTY_LIST;
  }

  @Override
  public void setValue(ApplicationWrapper node, String[] path, StatesDynamic value) {}
  ...
  @Override
  public StatesObject getExtraInfo(ApplicationWrapper node) {
    return new StatesObject.Builder().put("hasAXNode", true).build();
  }
