...
#import "StatesCppWrapperPlugin.h"
...
using WrapperPlugin = facebook::states::StatesCppWrapperPlugin;

@implementation StatesClient {
  facebook::states::StatesClient *_cppClient;
...
{
  static StatesClient *sharedClient = nil;
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    try {
      facebook::states::StatesClient::init({
        {
          "localhost",
          "iOS",
      ...
      _cppClient = facebook::states::StatesClient::instance();
...
- (NSArray *)getStateElements {
  ...
  for (facebook::states::StateElement element: _cppClient->getStateElements()) {
    facebook::states::State state = element.state_;
    NSString *stateString;
    switch (state) {
      case facebook::states::in_progress:
        stateString = @"⏳ ";
        break;

      case facebook::states::success:
        stateString = @"✅ ";
        break;

      case facebook::states::failed:
        stateString = @"❌ ";
        break;
...
@implementation StatesClient (Testing)
