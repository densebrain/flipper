...
@property (nonatomic, strong) id<StatoConnection> statoConnection;
...
- (void)didConnect:(id<StatoConnection>)connection {
    self.statoConnection = connection;
    [connection receive:@"getAllSharedPreferences" withBlock:^(NSDictionary *params, id<StatoResponder> responder) {
    ...
    [connection receive:@"getSharedPreferences" withBlock:^(NSDictionary *params, id<StatoResponder> responder) {
        [responder success:[self.userDefaults dictionaryRepresentation]];
    }];
    
    [connection receive:@"setSharedPreference" withBlock:^(NSDictionary *params , id<StatoResponder> responder) {
...
- (void)userDefaultsChangedWithValue:(id)value key:(NSString *)key {
    ...
    [self.statoConnection send:@"sharedPreferencesChange" withParams:[params copy]];
}
