...
#pragma clang diagnostic ignored "-Wundeclared-selector"
        _forwardingIMP = class_getMethodImplementation([NSObject class], @selector(statoKitThisMethodShouldNotExist));
...
- (void)swizzleSelector:(SEL)selector class:(Class)aClass block:(void (^)(NSInvocation * _Nonnull))blk {
    if (![self.swizzledClasses containsObject:aClass]) {
        ...
        IMP imp = imp_implementationWithBlock(^(id this, NSInvocation *invocation) {
            ...
            if (blk != nil) {
                NSString *originalStr = [@"comfacebookStatoKit_" stringByAppendingString:selStr];
    ...
    NSString *newSelStr = [@"comfacebookStatoKit_" stringByAppendingString:NSStringFromSelector(selector)];
