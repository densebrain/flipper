...
@property(assign, nonatomic) std::shared_ptr<facebook::states::DispatchQueue> connectionAccessQueue;
...
//   std::shared_ptr<facebook::states::DispatchQueue> _connectionAccessQueue;
//
...
- (instancetype)initWithQueue:(dispatch_queue_t)queue {
  if (self = [super init]) {
    ...
    _connectionAccessQueue = std::make_shared<facebook::states::GCDQueue>(queue);
  }
  return self;
}
...
- (instancetype)initWithVectorEventSize:(NSUInteger)size connectionAccessQueue:(std::shared_ptr<facebook::states::DispatchQueue>)connectionAccessQueue {
    if (self = [super init]) {
...
- (instancetype)initWithDispatchQueue:(std::shared_ptr<facebook::states::DispatchQueue>)queue {
