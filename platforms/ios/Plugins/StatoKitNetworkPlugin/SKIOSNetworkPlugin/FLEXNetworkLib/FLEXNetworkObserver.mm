...
@property (nonatomic, strong) NSMutableDictionary<NSString *, FLEXInternalRequestState *> *requestStatoForRequestIDs;
...
+ (void)injectIntoNSURLConnectionCancel
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        class_addMethod(className, swizzledSelector, implementation, method_getTypeEncoding(originalCancel));
...
+ (void)injectIntoNSURLSessionTaskResume
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (![[NSProcessInfo processInfo] respondsToSelector:@selector(operatingSystemVersion)]) {
            className = NSClassFromString([@[@"__", @"NSC", @"FLocalS", @"ession", @"Task"] componentsJoinedByString:@""]);
        } else if ([[NSProcessInfo processInfo] operatingSystemVersion].majorVersion < 9) {
            className = [NSURLSessionTask class];
        } else {
            className = NSClassFromString([@[@"__", @"NSC", @"FURLS", @"ession", @"Task"] componentsJoinedByString:@""]);
        }
        ...
        IMP implementation = imp_implementationWithBlock(swizzleBlock);
        class_addMethod(className, swizzledSelector, implementation, method_getTypeEncoding(originalResume));
...
+ (void)injectIntoNSURLConnectionAsynchronousClassMethod
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        void (^asyncSwizzleBlock)(Class, NSURLRequest *, NSOperationQueue *, NSURLConnectionAsyncCompletion) = ^(Class slf, NSURLRequest *request, NSOperationQueue *queue, NSURLConnectionAsyncCompletion completion) {
                ...
                NSString *mechanism = [self mechanismFromClassMethod:selector onClass:className];
        ...
        [FLEXUtility replaceImplementationOfKnownSelector:selector onClass:className withBlock:asyncSwizzleBlock swizzledSelector:swizzledSelector];
    });
}
...
+ (void)injectIntoNSURLConnectionSynchronousClassMethod
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        [FLEXUtility replaceImplementationOfKnownSelector:selector onClass:className withBlock:syncSwizzleBlock swizzledSelector:swizzledSelector];
    });
}
...
+ (void)injectIntoNSURLSessionAsyncDataAndDownloadTaskMethods
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            if ([FLEXUtility instanceRespondsButDoesNotImplementSelector:selector class:className]) {
            ...
            [FLEXUtility replaceImplementationOfKnownSelector:selector onClass:className withBlock:asyncDataOrDownloadSwizzleBlock swizzledSelector:swizzledSelector];
        }
    });
}
...
+ (void)injectIntoNSURLSessionAsyncUploadTaskMethods
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            [FLEXUtility replaceImplementationOfKnownSelector:selector onClass:className withBlock:asyncUploadTaskSwizzleBlock swizzledSelector:swizzledSelector];
        }
    });
}
...
{
    return [NSString stringWithFormat:@"+[%@ %@]", NSStringFromClass(className), NSStringFromSelector(selector)];
}
...
- (id)init
{
    self = [super init];
    if (self) {
        self.requestStatoForRequestIDs = [NSMutableDictionary new];
...
{
    FLEXInternalRequestState *requestState = self.requestStatoForRequestIDs[requestID];
    if (!requestState) {
        requestState = [FLEXInternalRequestState new];
        [self.requestStatoForRequestIDs setObject:requestState forKey:requestID];
    }
    return requestState;
}
...
{
    [self.requestStatoForRequestIDs removeObjectForKey:requestID];
}
