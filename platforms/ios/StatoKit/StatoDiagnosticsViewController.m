...
static NSString *const kSKCellIdentifier = @"StatoDiagnosticStateTableStableCellIdentifier";
...
@implementation StatoDiagnosticsViewController
...
- (void)viewDidLoad {
  ...
  self.tableDataSource = [[StateTableDataSource alloc] initWithElements:[[StatoClient sharedClient] getStateElements]];
...
- (void)onUpdate {
  StatoDiagnosticsViewController __weak *weakSelf = self;
...
- (void)updateStateTable {
  self.tableDataSource.elements = [[StatoClient sharedClient] getStateElements];
  [self.stateTable reloadData];
}

- (void)updateLogView {
  NSString *state = [[StatoClient sharedClient] getState];
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  id<StatoStateUpdateListener> weakSelf = self;
  [[StatoClient sharedClient] subscribeForUpdates:weakSelf];
}
