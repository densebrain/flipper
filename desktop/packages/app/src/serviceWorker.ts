...
self.addEventListener("fetch", (event: any) => {
  ...
  if (isFetchEvent(event) && event.request.url.startsWith("https://external.xx.fbcdn.net/assets/")) {
    event.respondWith(
      ...
      caches.match(event.request).then(cacheResponse => {
        return (
          ...
          fetch(event.request).then(response => {
            const clone = response.clone() // write to cache
...
function isFetchEvent(event: any) {
