 ...
 * @format
 */
const { resolve, dirname } = require("path") // do not apply this transform for these paths

const EXCLUDE_PATHS = ["/node_modules/react-devtools-core/", "relay-devtools/DevtoolsUI"]

function isExcludedPath(path: any) {
    ...
    CallExpression(path: any, state: any) {
      ...
      if (node.callee.name === "require" && args.length === 1 && t.isStringLiteral(args[0])) {
        if (args[0].value === "stato") {
          ...
          args[0].value.indexOf("/") > -1 && // in the plugin itself and not inside one of its dependencies
          state.file.opts.filename.indexOf("node_modules") === -1 && // the resolved path for this file is outside the plugins root
          !resolve(dirname(state.file.opts.filename), args[0].value).startsWith(state.file.opts.root) &&
          !(resolve(dirname(state.file.opts.filename), args[0].value).indexOf("/static/") < 0)
        ) {
          throw new Error(
