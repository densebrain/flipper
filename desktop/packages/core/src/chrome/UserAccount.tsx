...
const Container = styled(FlexRow)(({theme:{colors}}: SimpleThemeProps) => ({
  alignItems: 'center',
  padding: '5px 10px',
...
const UserAccount = withTheme()(class UserAccount extends React.PureComponent<UserAccountProps> {
  ...
  showDetails = () => {
    ...
    const {
      bottom = 0,
      left = 0
    } = this._ref ? this._ref.getBoundingClientRect() : {} as ClientRect;
    menu.popup({
  ...
  render() {
    ...
    return this.props.user.name ? <Container innerRef={this.setRef} onClick={this.showDetails}>
      ...
      </Container> : <Container onClick={() => this.props.setActiveSheet(ACTIVE_SHEET_SIGN_IN)}>
        <Glyph name="profile-circle" size={16} variant="outline" color={colors.text} />
        &nbsp;Sign In...
      </Container>;
  }
