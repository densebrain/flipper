...
const DialogContainer = styled('div')(({
  ...
  transform: `translate(-50%, ${state === 'entering' || state === 'exiting' || state === 'exited' ? 'calc(-100% - 20px)' : '0%'})`,
...
const Sheet = withTheme()(class Sheet extends Component<AllProps, State> {
  state: State = {
  ...
  render() {
    return <Transition in={Boolean(this.props.activeSheet) && this.state.isVisible} timeout={300} onExited={() => this.props.onHideSheet()}>
        {state => <DialogContainer state={state}>
            <div
        /* This is the target for React.portal, it should not be
         * unmounted, therefore it's hidden, when another sheet
         * is presented. */
        id={PLUGIN_SHEET_ELEMENT_ID} style={{
          display: this.props.activeSheet === ACTIVE_SHEET_PLUGIN_SHEET ? 'block' : 'none'
        }} />
