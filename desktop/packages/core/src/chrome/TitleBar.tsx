...
const AppTitleBar = styled(FlexRow)(
  styleCreator((props: AppTitleBarProps) => ({
...
export class TitleBarNaked extends React.Component<Props> {
  render() {
    ...
    return (
      <AppTitleBar focused={windowIsFocused} className="toolbar">
        <DevicesButton />
        <ScreenCaptureButtons />
        {downloadingImportData && (
          <Importing className="stato-titlebar">
            <LoadingIndicator size={16} />
            &nbsp;Importing data...
          </Importing>
        )}
        <Spacer className="stato-titlebar" />
        <VersionText className="stato-titlebar">
          {version}
          {isProduction() ? "" : "-dev"}
        </VersionText>
        ...
        {isAutoUpdaterEnabled() ? (
          ...
          <UpdateIndicator launcherMsg={launcherMsg} />
        )}
        {config.bugReportButtonVisible && (
          <Button
            compact={true}
            onClick={() => setActiveSheet(ACTIVE_SHEET_BUG_REPORTER)}
            title="Report Bug"
            icon="bug"
          />
        )}
