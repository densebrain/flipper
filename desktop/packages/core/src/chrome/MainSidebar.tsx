...
import {ActiveSheet, setActiveSheet} from '../reducers/application'
...
const PluginSidebarListItem = withStyles(baseStyles, {
  withTheme: true
})(class extends React.Component<PluginSidebarListItemProps> {
  render() {
    const {
      isActive,
      plugin,
...
const MainSidebar = withStyles(baseStyles, {
  ...
  render() {
    ...
    const byPluginNameOrId = (a:Plugin, b:Plugin) => (a.title || a.id) > (b.title || b.id) ? 1 : -1
    
    return <Sidebar backgroundColor={lighten(colors.background, 0.1)} position="left" width={200}>
      <Plugins>
        {!GK.get('flipper_disable_notifications') &&
        <ListItem active={selectedPlugin === 'notifications'} onClick={() => selectPlugin({
          selectedPlugin: 'notifications',
          selectedApp: null,
          deepLinkPayload: null
        })}>
          <PluginIcon color={colors.text} name={numNotifications > 0 ? NotificationsHub.icon || 'bell' : 'bell-null'}
        ...
        {selectedDevice && <FlexColumn className={classes.list}>
          {Array.from(this.props.devicePlugins.values()).filter(plugin => isDevicePlugin(plugin) && plugin.componentClazz.supportsDevice(selectedDevice)).sort(byPluginNameOrId).map((plugin:Plugin) =>
            <PluginSidebarListItem key={plugin.id} isActive={plugin.id === selectedPlugin} onClick={() => selectPlugin({
        ...
        {clients.filter((client:Client) => selectedDevice && client.query.device_id === selectedDevice.serial || // Old android sdk versions don't know their device_id
          ...
          {Array.from(this.props.clientPlugins.values()).filter((p:Plugin) => client.plugins.indexOf(p.id) > -1).sort(byPluginNameOrId).map((plugin:Plugin) =>
            <PluginSidebarListItem key={plugin.id} isActive={plugin.id === selectedPlugin && selectedApp === client.id}
