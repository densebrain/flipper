...
export default (store: Store, logger: Logger) => {
  ...
  server.addListener("removed-client", (id: string) => {
    ...
    store.dispatch({
      ...
      payload: {
        id,
        devicePlugins: new Set([...store.getState().plugins.devicePlugins.keys()])
      }
    })
  })
  ...
  server.addListener("finish-client-setup", (payload: { client: UninitializedClient, deviceId: string }) => {
    store.dispatch({
      type: "FINISH_CLIENT_SETUP",
      payload: payload
    })
  })
  server.addListener("client-setup-error", (payload: { client: UninitializedClient, error: Error }) => {
    store.dispatch({
