 ...
 * @format
 */
import { ipcRenderer } from "electron" // $FlowFixMe perf_hooks is a new API in node
...
export default async function (store: Store, logger: Logger) {
  ...
  function droppedFrameDetection(past: DOMHighResTimeStamp, isWindowFocused: () => boolean) {
  ...
  if (typeof window !== "undefined") {
    droppedFrameDetection(performance.now(), () => store.getState().application.windowIsFocused)
  }

  ipcRenderer.on("trackUsage", () => {
    const { selectedDevice, selectedPlugin, selectedApp, clients } = store.getState().connections
    ...
    if (selectedApp) {
      ...
      if (client) {
        ...
        sdkVersion = "" + (client.query.sdkVersion || 0)
      }
    }
