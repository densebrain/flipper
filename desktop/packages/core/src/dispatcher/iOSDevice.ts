...
type IOSDeviceParams = {
  udid: string,
  type: DeviceType,
  name: string
}
const portforwardingClient = isProduction()
  ? path.resolve(__dirname, "PortForwardingMacApp.app/Contents/MacOS/PortForwardingMacApp")
...
function queryDevices(store: Store, logger: Logger): Promise<void> {
  ...
  const currentDeviceIDs: Set<string> = new Set(
    connections.devices.filter(device => device instanceof IOSDevice).map(device => device.serial)
  )
...
function getActiveSimulators(): Promise<Array<IOSDeviceParams>> {
  const deviceSetPath = process.env.DEVICE_SET_PATH ? ["--set", process.env.DEVICE_SET_PATH] : []
  return promisify(execFile)("xcrun", ["simctl", ...deviceSetPath, "list", "devices", "--json"], {
    encoding: "utf8"
  })
    .then(({ stdout }) => JSON.parse(stdout).devices as {[type: string]: Array<iOSSimulatorDevice>})
    .then(simulatorDevices => {
      const simulators = Object.values(simulatorDevices)
        .reduce((acc, cv) => acc.concat(...cv), [] as Array<iOSSimulatorDevice>)
      return simulators
        .filter(
          simulator =>
            simulator.state === "Booted" && // For some users "availability" is set, for others it's "isAvailable"
