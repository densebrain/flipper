...
function createDevice(adbClient: any, device: any, store: Store): Promise<AndroidDevice> {
  return new Promise((resolve, _reject) => {
    const type = device.type !== "device" || device.id.startsWith("emulator") ? "emulator" : "physical"
    adbClient.getProperties(device.id).then(async (props:any) => {
...
function getRunningEmulatorName(id: string): Promise<string | null | undefined> {
  return new Promise((resolve, reject) => {
    const port = id.replace("emulator-", "") // The GNU version of netcat doesn't terminate after 1s when
  ...
  const watchAndroidDevices = () => {
    ...
    promisify(which)("emulator").then(emulatorPath => {
      child_process.exec(
        ...
        (error: Error | null | undefined, data: string | null | undefined) => {
