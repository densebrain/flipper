...
export default function reducer(state: State = INITIAL_STATE, action: Action): State {
  switch (action.type) {
    ...
    case "CLEAR_ALL_NOTIFICATIONS":
      const markAsCleared = ({ pluginId, notification: { id } }: PluginNotification) =>
        state.clearedNotifications.add(`${pluginId}#${id}`)

      state.activeNotifications.forEach(markAsCleared)
      state.invalidatedNotifications.forEach(markAsCleared) // Q: Should this actually delete them, or just invalidate them?

      return { ...state, activeNotifications: [], invalidatedNotifications: [] }

    case "UPDATE_PLUGIN_BLACKLIST":
      return { ...state, blacklistedPlugins: action.payload }

    case "UPDATE_CATEGORY_BLACKLIST":
      return { ...state, blacklistedCategories: action.payload }
