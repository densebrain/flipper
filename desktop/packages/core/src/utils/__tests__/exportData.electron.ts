...
import {FlipperDevicePluginComponent} from "../../plugin"
import {makeDevicePlugin, Notification} from "../../PluginTypes"
import {ClientExport} from "../../Client"
import {Device} from "../../index"

const TestDevicePlugin = makeDevicePlugin(class TestDevicePluginComponent extends FlipperDevicePluginComponent {
...
test("test processStore function for an iOS device connected", async () => {
  ...
  expect(json).toBeDefined() // $FlowFixMe Flow doesn't that its a test and the assertion for null is already done
...
test("test processStore function to have only the client for the selected device", async () => {
  ...
  expect(json).toBeDefined() //$FlowFixMe Flow doesn't that its a test and the assertion for null is already added
