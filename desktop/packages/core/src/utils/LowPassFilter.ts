...
export default class LowPassFilter {
  ...
  next(nextValue: number): number {
    ...
    const removed = this.push(nextValue) // smooth value using all values from buffer

    const result = this.buffer.reduce(this._nextReduce, removed) // replace smoothed value
