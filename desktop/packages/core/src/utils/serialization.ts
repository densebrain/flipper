...
export function makeObjectSerializable(obj: any): any {
  ...
  return Object.entries(obj).reduce((acc, [key, value]) => ({ ...acc, [key]: makeObjectSerializable(value) }), {})
}
export function deserializeObject(obj: any): any {
  ...
  if (Array.isArray(obj)) {
    ...
    switch (type) {
      case "Map": {
        return new Map(obj.data.map((item: Array<any>) => [...item].map(deserializeObject)) as any)
      }
  ...
  return Object.entries(obj).reduce((acc, [key, value]) => ({ ...acc, [key]: deserializeObject(value) }), {})
}
