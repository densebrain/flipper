 ...
 * certificates to allow for secure communication between Stato and apps.
 * It takes a Certificate Signing Request which was generated by the app,
 * using the app's public/private keypair.
 * With this CSR it uses the Stato CA to sign a client certificate which it
 * deploys securely to the app.
 * It also deploys the Stato CA cert to the app.
...
export default class CertificateProvider {
  ...
  async executeCommandOnAndroid(
    ...
    if (output.match(appNotDebuggableRegex)) {
      const e = new Error(
        `Android app ${user} is not debuggable. To use it with Stato, add android:debuggable="true" to the application section of AndroidManifest.xml`
      )
      this.server.emit("error", e)
      throw e
    }
