...
export default class IOSDevice extends BaseDevice {
  ...
  startLogListener(retries: number = 3) {
    ...
    if (!this.log) {
      ...
      this.log.on("error", (err:any) => {
        console.error(err)
      })
      this.log.stderr.on("data", (data:any) => {
    ...
    try {
      ...
      console.error("Could not parse iOS log stream.", e) // restart log stream
  ...
  static parseLogEntry(entry: RawLogEntry): DeviceLogEntry {
    ...
    let type: LogLevel = LOG_MAPPING.get(entry.messageType) || "unknown" // when Apple log levels are not used, log messages can be prefixed with
    // their loglevel.
...
class StripLogPrefix extends Transform {
  ...
  _transform(data: any, _encoding: string, callback: (err?: Error, data?: any) => void) {
