...
export default class Orderable extends React.Component<OrderableProps, OrderableState> {
  ...
  _mousemove: ((event:MouseEvent) => void) | null | undefined
  _mouseup: ((event:MouseEvent) => void) | null | undefined
  timer: any
  ...
  startMove = (key: string, event: React.MouseEvent<HTMLDivElement>) => {
    if (this.props.altKey === true && event.altKey === false) {
      return
    }

    if (this.props.ignoreChildEvents === true) {
      const tabRef = this.tabRefs[key] // $FlowFixMe parentNode not implemented

      
      if (event.target !== tabRef && event.currentTarget.parentNode !== tabRef) {
        return
      }
    }
  ...
  possibleMove(activeKey: string, goingOpposite: boolean, event: MouseEvent, cursorOffset: number, sizes: TabSizes) {
  ...
  moveTabs(order: OrderableOrder, activeKey: string | null | undefined, sizes: TabSizes) {
    let offset = 0
  ...
  addRef = (key: string, elem: HTMLElement | null | undefined) => {
  ...
  render() {
    const { items } = this.props // calculate order of elements
    ...
    return (
      <OrderableContainer className={this.props.className} innerRef={this.setContainerRef}>
        {order.map(key => {
