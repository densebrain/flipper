...
import { PluginClient } from "../../PluginTypes"
type ValueWithType = {
  type: string,
  value: any
}
...
type CommandResultFailed = {
  isSuccess: false,
  error: string
}
type CommandResult = CommandResultSuccess | CommandResultFailed

function isCommandResultSuccess(result: CommandResult): result is CommandResultSuccess {
...
class ConsoleError extends React.Component<{
  ...
  render() {
    ...
    return <ConsoleError.Container className={className}>{error}</ConsoleError.Container>
  }
}
...
export class Console extends React.Component<Props, State> {
  ...
  static buildCommandResultRowPair(command: string, result: CommandResult, index: number): TableRows {
    const key = index * 2
    ...
    const resultRow: TableBodyRow = {
      columns: {
        command: {
          value: isCommandResultSuccess(result) ? (
            <ManagedDataInspector data={result.value} expandRoot={true} collapsed={true} />
          ) : (
  ...
  renderPreviousCommands() {
    const rows: TableRows = this.state.previousExecutions
      .map(({ command, result }, index) => Console.buildCommandResultRowPair(command, result, index))
    ...
    return (
      <Console.Window>
        ...
        <form onSubmit={this.onSubmit}>
          <Console.Input onChange={this.onInputChange} placeholder="Command" value={this.state.script} />
        </form>
      </Console.Window>
    )
  }
