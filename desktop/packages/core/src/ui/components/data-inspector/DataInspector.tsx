...
export type DataValueMetadata = {
  mutable: boolean
  type: string
  value: any
}
export type DataValueExtractor<T = any> = (
  value: T,
  depth: number
) =>
  | DataValueMetadata
...
type DataInspectorProps<T = any> = {
  ...
  extractValue?: DataValueExtractor<T>,
  onExpanded?: (expanded: DataInspectorExpanded) => void | null | undefined,
  setValue?: DataInspectorSetValue | null | undefined,
  collapsed?: boolean,
  ancestry: Array<Object>,
  tooltips?: {[name: string]: Tooltip}
}
...
const rootContextMenuCache: WeakMap<Object, Array<Electron.MenuItemConstructorOptions>> = new WeakMap()

function getRootContextMenu(data: Object): Array<Electron.MenuItemConstructorOptions> {
...
function isPureObject(obj: Object) {
  return obj !== null && Object.prototype.toString.call(obj) !== "[object Date]" && typeof obj === "object"
}
...
export default class DataInspector<T = any> extends Component<DataInspectorProps<T>> {
  static defaultProps: Partial<DataInspectorProps> = {
    expanded: {},
    depth: 0,
    path: [],
    ancestry: []
  ...
  private interaction: (name: string) => void

  constructor(props: DataInspectorProps<T>) {
    super(props)
  ...
  shouldComponentUpdate(nextProps: DataInspectorProps) {
    const { props } = this // check if any expanded paths effect this subtree
  ...
  isExpanded(pathParts: Array<string>) {
    const { expanded } = this.props // if we no expanded object then expand everything

    if (expanded == null) {
      return true
    }

    const path = pathParts.join(".") // check if there's a setting for this path
  ...
  setExpanded(pathParts: Array<string>, isExpanded: boolean) {
    ...
    onExpanded({ ...expanded, [path]: isExpanded })
  }
  ...
  extractValue = (data: T, depth: number) => {
    let res
  ...
  render(): any {
    const {
      ...
      collapsed,
      tooltips
    } = this.props // the data inspector makes values read only when setValue isn't set so we just need to set it
    ...
    const isExpanded =
      isExpandable &&
      (resDiff != null
        ? isComponentExpanded(value, resDiff.type, resDiff.value, expandRoot === true || this.isExpanded(path))
    ...
    if (isExpandable && isExpanded) {
      const propertyNodes = [] // ancestry of children, including its owner object
    ...
    if (isString(name)) {
      nameElems.push(
        <Tooltip title={tooltips != null && tooltips[name]} key="name" options={nameTooltipOptions}>
    ...
    if (isExpanded || !isExpandable) {
      descriptionOrPreview = <DataDescription path={path} setValue={setValue} type={type} value={value} />
    } else {
      descriptionOrPreview = (
        <DataPreview type={type} value={value} extractValue={this.extractValue} depth={depth} />
      )
    }
    ...
    const contextMenuItems:Array<Electron.MenuItemConstructorOptions> = []
