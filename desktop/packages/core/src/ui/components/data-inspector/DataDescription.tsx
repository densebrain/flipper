...
class NumberTextEditor extends PureComponent<{
  ...
  onNumberTextInputKeyDown = (e: React.KeyboardEvent<any>) => {
    if (e.key === "Enter") {
  ...
  onNumberTextRef = (ref: HTMLElement | null | undefined) => {
    if (ref) {
      ref.focus()
    }
  }
  ...
  render() {
    const extraProps: {type?: string, step?: string} = {}

    if (this.props.type === "number") {
      // render as a HTML number input
      extraProps.type = "number" // step="any" allows any sort of float to be input, otherwise we're limited
      // to decimal
...
export default class DataDescription extends PureComponent<DataDescriptionProps, DataDescriptionState> {
  ...
  _renderEditing() {
    ...
    if (type === "string" || type === "text" || type === "number" || type === "enum") {
      return <NumberTextEditor type={type} value={value} origValue={origValue} commit={this.commit} />
    }
  ...
  _hasEditUI() {
    const { type } = this.props
    return type === "string" || type === "text" || type === "number" || type === "enum" || type === "color"
  }
  ...
  render() {
    ...
    return (
      <ColorPickerDescription>
        <DataDescriptionPreview type="color" value={this.props.value} editable={false} commit={this.props.commit} />
...
class DataDescriptionPreview extends Component<{
  ...
  render() {
    ...
    const description = (
      <DataDescriptionContainer type={type} value={value} editable={this.props.editable} commit={this.props.commit} />
...
function parseColor(
  ...
  val = val.slice(1) // only allow RGB and ARGB hex values
  ...
  let a = 1 // extract alpha if passed AARRGGBB
...
class DataDescriptionContainer extends Component<{
  ...
  render(): any {
    ...
    switch (type) {
      ...
      case "boolean":
        return editable ? (
          <input type="checkbox" checked={Boolean(val)} disabled={!editable} onChange={this.onChangeCheckbox} />
        ) : (
