...
export type RootViewProps<ElementType = any> = React.HTMLAttributes<
  ElementType
> & {
  innerRef?: React.Ref<ElementType> | React.RefObject<ElementType> | undefined
}
export function RootView<Props extends RootViewProps = RootViewProps>(
  props: Props
): React.ReactElement<Props, any> {
  const { className, children, ...other } = props as any
  return (
    <div className={className} {...filterProps(other)}>
      {children}
    </div>
  )
}

export type RootViewComponentType<Props extends RootViewProps> =
  | React.ComponentType<Props>
  | (
      | ((props: Props) => React.ReactElement<Props, any> | React.ReactNode)
      | React.ReactElement<Props, any>)
...
>(
  ...
  component: C = RootView as C,
  stylerOrName: RootViewStyler<ViewProps> | null | undefined | string = null,
  styler: RootViewStyler<ViewProps> | null | undefined = null
): C {
  //if (!Component)
  ...
  const memoStyler = memoFn(
    ...
    view = withStyles(
      ...
      React.forwardRef<C, ViewProps>((props, ref) => {
        let { style, className, classes, theme, ...other } = props
