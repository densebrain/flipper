...
export class InspectorSidebar extends Component<Props, State> {
  ...
  render() {
    ...
    const sections: Array<any> =
      (extensions &&
        extensions.map(ext => ext(this.props.client, this.props.realClient, oc(element).id(null), this.props.logger))) ||
      []
      ...
      for (const key in element.data) {
        if (key === "Extra Sections") {
          for (const extraSection of Object.keys(element.data[key])) {
            let data = element.data[key][extraSection] // data might be sent as stringified JSON, we want to parse it for a nicer persentation.
