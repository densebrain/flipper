...
export function makeDimensionConstraint<P extends any>(dim:'width' | 'height', val:string | number | CSSPropFn<P>):CSSProperties {
  ...
  return {
    [dim]: val, [`min${dimUpper}`]: val, [`max${dimUpper}`]: val, overflow: 'hidden'
  } as CSSProperties
...
export function makeDimensionConstraints<P extends any>(width:string | number | CSSPropFn<P>, height:string | number | CSSPropFn<P> = width):CSSProperties {
  return {
    ...makeHeightConstraint(height), ...makeWidthConstraint(width)
  }
}
...
export const FillWindow = Object.assign(makeWidthConstraint('100vw'), makeHeightConstraint('100vh')) // noinspection
...
export const PaddingProps:Array<string> = Object.keys(makePaddingRem(0))
                                                .map(key => key.replace(/([A-Z])/g, g => `-${g[0].toLowerCase()}`)) // noinspection
                                                                                                                     // JSUnusedGlobalSymbols

export const MarginProps:Array<string> = Object.keys(makeMarginRem(0))
                                               .map(key => key.replace(/([A-Z])/g, g => `-${g[0].toLowerCase()}`))
