 ...
 * @format
 */
import { ImageId, ImageData, ImagesList } from "./api"
...
type ImagesCacheOverviewState = {
  selectedImage: ImageId | null | undefined,
  size: number
}
...
export default class ImagesCacheOverview extends PureComponent<ImagesCacheOverviewProps, ImagesCacheOverviewState> {
  ...
  render() {
    ...
    return (
      <ImagesCacheOverview.Container grow={true} onKeyDown={this.onKeyDown} tabIndex="0">
        <Toolbar position="top">
          ...
          />
          <StyledCheckbox checked={this.props.coldStartFilter} onChange={this.props.onColdStartChange} />
          Show ColdStart Images
          <Spacer />
          <input type="range" onChange={this.onChangeSize} min={50} max={150} value={this.state.size} />
        </Toolbar>
        {!hasImages ? (
  ...
  render() {
    return (
      <ImageGridHeader.Container>
        ...
        <ImageGridHeader.Subtitle>{this.props.subtitle}</ImageGridHeader.Subtitle>
        <Spacer />
        {this.props.onClear ? (
          <ImageGridHeader.ClearButton onClick={this.props.onClear}>Clear Cache</ImageGridHeader.ClearButton>
        ) : null}
      </ImageGridHeader.Container>
    )
  }
  ...
  render() {
    ...
    return (
      <ImageItem.Container onClick={this.onClick} size={size}>
        {numberOfRequests > 0 && image != null && <ImageItem.Events>{numberOfRequests}</ImageItem.Events>}
        {image != null ? <ImageItem.Image src={image.data} /> : <LoadingIndicator size={25} />}
        <ImageItem.SelectedHighlight selected={selected} />
        {image != null && (
          <ImageItem.HoverOverlay selected={selected} size={size}>
            <ImageItem.MemoryLabel>{formatKB(image.sizeBytes)}</ImageItem.MemoryLabel>
            <ImageItem.SizeLabel>
