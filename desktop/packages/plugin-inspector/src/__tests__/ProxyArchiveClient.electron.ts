...
function populateChildren(state: PersistedState, axMode: boolean) {
  ...
  elements["root"] = constructElement("root", "root view", ["child0", "child1"])
  elements["child0"] = constructElement("child0", "child0 view", ["child0_child0", "child0_child1"])
  elements["child1"] = constructElement("child1", "child1 view", ["child1_child0", "child1_child1"])
  elements["child0_child0"] = constructElement("child0_child0", "child0_child0 view", [])
  elements["child0_child1"] = constructElement("child0_child1", "child0_child1 view", [])
  elements["child1_child0"] = constructElement("child1_child0", "child1_child0 view", [])
  elements["child1_child1"] = constructElement("child1_child1", "child1_child1 view", [])
...
test("test the searchNode for root in axMode false", async () => {
  let searchResult: SearchResultTree | null | undefined = await searchNodes(
    state.elements["root"],
    "root",
    false,
    state
...
test("test the searchNode for root in axMode true", async () => {
  ...
  expect(searchResult).toEqual({
    ...
    hasChildren: false,
    children: null,
    element: state.AXelements["root"],
    // Even though AXElement exists, normal element will exist too
...
test("test the searchNode for which matches multiple child", async () => {
  ...
  expect(searchResult).toBeDefined()
  let expectedSearchResult:SearchResultTree = {
    id: "root",
    isMatch: false,
    hasChildren: true,
    children: [
...
test("test different methods of calls with no params", async () => {
  let proxyClient = new ProxyArchiveClient(state)
  await expect(proxyClient.call("getNodes")).rejects.toThrow(new Error("Called getNodes with no params"))
  await expect(proxyClient.call("getAXNodes")).rejects.toThrow(new Error("Called getAXNodes with no params")) // let result: Error = await proxyClient.call('getSearchResults');
  ...
  ).rejects.toThrow(new Error("query is not passed as a params to getSearchResults"))
})
