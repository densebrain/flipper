...
import {
  ...
  PluginType,
  Plugin,
  PluginModuleExport,StatoPluginProps, Store
} from "@stato/core"
import { FlexColumn, FlexRow,StatoPluginComponent, Toolbar, Sidebar, Link, Glyph, DetailSidebar, styled } from "@stato/core"
...
class LayoutPlugin extends StatoPluginComponent<Props, State, {}, PersistedState> {
  ...
  init() {
    ...
    if (!this.props.persistedState) {
      // If the selected plugin from the previous session was layout, then while importing the stato trace, the redux store doesn't get updated in the first render, due to which the plugin crashes, as it has no persisted state
