...
const BEGIN_DETAILS_SECTION_INDICATOR = "* Details:" // Line following the end of the Details section

const END_DETAILS_SECTION_INDICATOR = "* Excluded Refs:"
const STATIC_PREFIX = "static " // Text that begins the line of the Object at GC root
...
function generateFieldsList(
  ...
  let instanceValues:any = {}
  let elementId = -1
  let elementIdStr = String(-1)
  let packages:any = {} // Process everything between Details and Excluded Refs
  ...
  while (i < lines.length && !lines[i].startsWith(END_DETAILS_SECTION_INDICATOR)) {
    ...
    if (line.startsWith("*")) {
      ...
      elementId++
      elementIdStr = String(elementId) // Extract package for each class
...
function processLeak(output: Leak[], leakInfo: string): Leak[] {
  ...
  i++ // Parse information on each object's fields, package

  const { staticFields, instanceFields, packages } = generateFieldsList(lines, i) // While elementsSimple remains as-is, elements has the package of each class
  // inserted, in order to enable 'Show full class path'

  Object.keys(packages).forEach((elementId:any) => {
    const pkg = (packages as any)[elementId]
    const simpleName = elements[elementId].name // Gets everything before the field name, which is replaced by the package
