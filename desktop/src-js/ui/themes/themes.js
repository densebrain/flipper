...
import type {Colors, Palette, BaseTheme, ThemeComponents, ThemeConfig} from './ThemeTypes';
...
function makeTheme(themeConfig:ThemeConfig) {
  ...
  const
    {palette, customizeTheme, customizeColors} = themeConfig,
  ...
  const getContrastText = (bg:ColorProperty) =>
    getContrastRatio(bg, text) >= contrastThreshold
      ? text
      : textInverted;
  
  const
    textSelected = getContrastText(backgroundSelected),
  ...
  let theme = {
    ...
    titlebar: ({focused}) => ({
      background: focused
        ? `linear-gradient(to bottom, ${colors.backgroundStatus} 0%, ${
          lighten(colors.backgroundStatus,0.1)
          } 100%)`
        : colors.backgroundStatus,
      borderBottom: `1px solid ${
        focused ? colors.border : colors.border
        }`
    }),
