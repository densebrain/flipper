...
export default (store: Store, logger: Logger) => {
  ...
  ipcRenderer.on(
    "notificationEvent",
    (e, eventName: NotificationEvents, pluginNotification: PluginNotification, arg: null | string | number) => {
  ...
  store.subscribe(() => {
    ...
    const clientPlugins: Map<string, Class<FlipperPlugin<>>> = store.getState().plugins.clientPlugins
    const devicePlugins: Map<string, Class<FlipperDevicePlugin<>>> = store.getState().plugins.devicePlugins
    const pluginMap: Map<string, Class<FlipperPlugin<> | FlipperDevicePlugin<>>> = new Map([
      ...clientPlugins,
      ...devicePlugins
    ])
    Object.keys(pluginStates).forEach(key => {
      if (knownPluginStates.get(key) !== pluginStates[key]) {
        ...
        const persistingPlugin: Class<FlipperPlugin<> | FlipperDevicePlugin<>> | null | undefined = pluginMap.get(
          pluginId
        )
        ...
        if (persistingPlugin && persistingPlugin.getActiveNotifications) {
          store.dispatch(
            setActiveNotifications({
              notifications: persistingPlugin.getActiveNotifications(pluginStates[key]),
              client,
              pluginId
            })
          )
