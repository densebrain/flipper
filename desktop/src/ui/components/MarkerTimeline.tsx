...
const Point = styled(FlexRow)(props => ({
  ...
  '::before': {
    ...
    marginRight: 6,
    boxShadow: props.selected ? `0 0 0 2px ${colors.macOSTitleBarIconSelected}` : null
  },
  '::after': {
...
export default class MarkerTimeline extends Component<Props, State> {
  ...
  static getDerivedStateFromProps(props: Props) {
    const sortedMarkers: Array<[number, Array<DataPoint>]> = Array.from(props.points.reduce((acc: Map<number, Array<DataPoint>>, cv: DataPoint) => {
  ...
  render() {
    ...
    return <Markers totalTime={timePoints[timePoints.length - 1].positionY + this.props.lineHeight}>
