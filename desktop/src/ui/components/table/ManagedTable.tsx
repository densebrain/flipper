...
import {
  ...
  TableBodyRow,
  TableOnAddFilter,
  TableRowAddress, isTableDataPage
} from './types'
...
class ManagedTable extends React.Component<ManagedTableProps, ManagedTableState> {
  static defaultProps = {
  ...
  makeManagedTableState(props:ManagedTableProps = this.props):ManagedTableState {
    return {
      columnOrder: JSON.parse(window.localStorage.getItem(this.getTableKey()) || 'null') || props.columnOrder || Object.keys(props.columns).map(key => ({
        key,
        visible: true
      })),
  ...
  onDataChange = (_newRows:Array<TableBodyRow>, _oldRows:Array<TableBodyRow>) => {
  ...
  onTableUpdated(highlightedRowsChanged:boolean = false) {
    ...
    if (shouldScrollToBottom && !newHighlightedRows.size) {
      ...
      const
        row = items.find(item => item.key === visibleStartKey),
        itemElement = document.querySelector(`[data-key=${row.key}]`) as HTMLElement | null //index = this.items.findIndex(row => row.key === this.state.visibleStartKey);
      const tableElement = this.tableRef.current
      if (itemElement && tableElement) {
        tableElement.scrollTo(itemElement.offsetTop + this.state.visibleStartOffset)
      }
    }
  ...
  componentDidUpdate(prevProps:ManagedTableProps,
    ...
    this.onTableUpdated(this.props.highlightedRows !== prevProps.highlightedRows) // if (this.props.highlightedRows !== prevProps.highlightedRows) {
    //
    // }
  ...
  onKeyDown = (e:KeyboardEvent) => {
    ...
    if ((e.metaKey && process.platform === 'darwin' || e.ctrlKey && process.platform !== 'darwin') && e.keyCode === 67) {
      this.onCopy()
    } else if (['ArrowUp', 'ArrowDown'].includes(e.code) && this.props.highlightableRows) {
      // arrow navigation
      const {
        items
      } = this
      ...
      if (this.isRowTypePage()) {
        ...
        const newItemIndex = newIndex.itemIndex = isUpKey ? // UP KEY ITEM INDEX
          rowIndex === 0 ? Math.max(0, itemIndex - 1) : itemIndex : // DOWN KEY ITEM INDEX
          rowIndex + 1 >= lastItemRows.length ? Math.min(items.length - 1, itemIndex + 1) : itemIndex
        const newItemRows = ((items[newItemIndex] as any) as ManagedTableDataPage<any>).getTableRows()
        newIndex.rowIndex = newItemIndex !== itemIndex ? isUpKey ? Math.max(newItemRows.length - 1, 0) : 0 : isUpKey ? Math.max(0, rowIndex - 1) : Math.min(newItemRows.length - 1, rowIndex + 1)
      } else {
        newIndex.itemIndex = Math.min(items.length - 1, Math.max(0, isUpKey ? itemIndex - 1 : itemIndex + 1))
      }
  ...
  onRowHighlighted = (highlightedRows:Set<string>, cb?: (() => void) | undefined) => {
  ...
  onHighlight = (e:React.MouseEvent, row:TableBodyRow, itemIndex:number, rowIndex:number = 0) => {
    ...
    if ((e.metaKey && process.platform === 'darwin' || e.ctrlKey) && this.props.multiHighlight) {
  ...
  selectInRange = (fromKey:string, toKey:string):Array<string> => {
    ...
    const isValid = (rowAddress: TableRowAddress) => rowAddress && rowAddress.itemIndex !== -1 && rowAddress.rowIndex !== -1
  ...
  getRowOffset(row:TableBodyRow):number | null | undefined {
    ...
    return !scroller ? null : getValue(() => scroller.querySelector(`[data-type="${row.key}"]`).scrollTop,null)
  }
  ...
  onMouseEnterRow = (_e:React.MouseEvent, row:TableBodyRow, _itemIndex:number, _rowIndex:number) => {
    const {
      dragStartIndex
    } = this
    const {
    ...
    if (dragStartIndex && table && this.props.multiHighlight && this.props.highlightableRows) {
  ...
  onScroll = debounce(({
    ...
    if (stickyBottom && scrollDirection === 'forward' && !shouldScrollToBottom && current && parent instanceof HTMLElement && parent.scrollTop + parent.clientHeight * 1.2 >= current.offsetHeight) {
      newState.shouldScrollToBottom = true
      newState.visibleStartKey = null
      newState.visibleStartOffset = 0
    } else if (this.props.stickyBottom && scrollDirection === 'backward' && this.state.shouldScrollToBottom) {
      newState.shouldScrollToBottom = false
    }
    
    if (newState.shouldScrollToBottom !== true && this.state.shouldScrollToBottom !== true && !scrollUpdateWasRequested && key && key !== this.state.visibleStartKey) {
      newState.visibleStartKey = key
      newState.visibleStartOffset = scrollOffset - child.clientHeight
    }
    
    if (Object.keys(newState).length) this.setState(state => ({...state,...newState}))
  }, 250)
  ...
  getRow = ({
    ...
    const columnKeys = columnOrder.map(k => k.visible ? k.key : null).filter(Boolean),
      ...
      tableElements = tableRows.map((row, rowIndex) => {
        return <TableRow key={row.key} className={row.className} columnSizes={columnSizes} columnKeys={columnKeys}
                         onMouseDown={(e: React.MouseEvent) => this.onHighlight(e, row, itemIndex, rowIndex)}
                         onMouseEnter={(e: React.MouseEvent) => this.onMouseEnterRow(e, row, itemIndex, rowIndex)} multiline={multiline}
                         rowLineHeight={24} highlighted={highlightedRows.has(row.key)} row={row} index={itemIndex}
                         onAddFilter={onAddFilter} zebra={zebra} {...this.isRowTypePage() ? {
          height: row.height
        } : {
          style
        }} />
  ...
  render() {
    ...
    return <Container>
      {hideHeader !== true && <TableHead columnOrder={columnOrder} onColumnOrder={this.onColumnOrder} columns={columns}
                                         onColumnResize={this.onColumnResize} sortOrder={this.state.sortOrder}
                                         columnSizes={columnSizes} onSort={this.onSort}/>}
      <Container>
        {autoHeight ? items.map((_, index) => this.getRow({
          index,
          isScrolling: false,
          style: {},
          data: items
          ...
          {({
              width,
              height
            }) => <ContextMenu buildItems={this.props.buildContextMenuItems || this.buildContextMenuItems}>
            <List itemData={items} itemCount={items.length} itemSize={this.itemSize} ref={this.tableRef}
                  itemKey={this.itemKey} width={width} overscanCount={0} innerRef={this.scrollRef}
                  onScroll={this.onScroll} height={height}>
              {this.getRow}
            </List>
          </ContextMenu>}
        </AutoSizer>}
