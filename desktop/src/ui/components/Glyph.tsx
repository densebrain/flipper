...
}))(({ classes, className, style, ...other }: GlyphProps& {classes: ThemedClassNames<"root">}) => (
  <img className={`${classes.root} ${className}`} style={style} {...filterProps(other)} />
))
const ColoredIconCustom = withStyles(() => ({
  ...
  <div className={`${classes.root} ${className}`} style={style} {...filterProps(other)} />
))
...
function ColoredIcon(
  ...
  const { className, color = className ? null : context.glyphColor, name, size = 16, src } = props
  const isBlack = !className && (color == null || color === "#000" || color === "black" || color === "#000000")

  if (isBlack) {
    return <ColoredIconBlack alt={name} src={src} size={size} className={className} />
  } else {
    return <ColoredIconCustom color={color} size={size} src={src} className={className} />
  }
}
...
export default class Glyph extends React.PureComponent<GlyphProps> {
  render() {
    ...
    return (
      <ColoredIcon name={name} className={className} color={color} size={size} src={getIconUrl(name, size, variant)} />
    )
  }
}
