...
const Content = makeRootView(
  () => ({
    alignItems: "flex-start",
    height: "100%",
    marginTop: (props:any) => props.top,
    minWidth: "100%",
    overflow: "visible"
  }),
  FlexColumn
...
export default class VirtualList extends Component<VirtualListProps, VirtualListState> {
  ...
  render() {
    ...
    const { height, offset } = this.state // first visible row index
    // eslint-disable-next-line no-bitwise

    let start = (offset / rowHeight) | 0 // actual number of visible rows (without overscan)
    // eslint-disable-next-line no-bitwise

    let visibleRowCount = (height / rowHeight) | 0 // Overscan: render blocks of rows modulo an overscan row count
    ...
    const end = start + 1 + visibleRowCount // data slice currently in viewport plus overscan items
    ...
    return (
      <View grow={true} onScroll={this.handleScroll} innerRef={this.setRef} scrollable={true}>
        {inner}
      </View>
    )
  }
