  ...
  render() {
    ...
    const columnSizes = Object.keys(COLUMNS).reduce((acc, cv) => {
      acc[cv] =
        COLUMNS[cv] === "flex"
          ? "flex"
          : children.reduce((acc, line) => Math.max(acc, line[cv]?.length || 0), 0) * 8 + 16 // approx 8px per character + 16px padding left/right
        ...
        <Container isCrash={this.props.isCrash} padded={this.props.padded}>
          {this.props.reason && (
            <Title isCrash={this.props.isCrash}>
              {this.props.isCrash && <Icon name="stop" variant="filled" size={16} color={colors.red} />}
