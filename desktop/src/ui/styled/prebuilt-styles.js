...
export function makeDimensionConstraint<P : any>(dim:'width'|'height',val:string|number|CSSPropFn<P>):CSSProperties {
...
export function makeHeightConstraint<P : any>(val:string|number|CSSPropFn<P>):CSSProperties {
  return makeDimensionConstraint<P>('height',val)
}

export function makeWidthConstraint<P : any>(val:string|number|CSSPropFn<P>):CSSProperties {
  return makeDimensionConstraint<P>('width',val)
}

// noinspection JSSuspiciousNameCombination
// noinspection JSUnusedGlobalSymbols,JSSuspiciousNameCombination
export function makeDimensionConstraints<P : any>(width:string|number|CSSPropFn<P>,height:string|number|CSSPropFn<P> = width):CSSProperties {
...
// noinspection JSUnusedGlobalSymbols
export const PaddingProps:Array<string> = Object.keys(makePaddingRem(0)).map(key => key.replace(/([A-Z])/g, (g) => `-${g[0].toLowerCase()}`));
...
export function remToPx(rem:number):number {
  return Math.round(rem * parseFloat(getComputedStyle((document.documentElement: any)).fontSize))
}
// noinspection JSUnusedGlobalSymbols
