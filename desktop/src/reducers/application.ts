...
export const ACTIVE_SHEET_PLUGIN_DEBUGGER: "PLUGIN_DEBUGGER" = "PLUGIN_DEBUGGER"
export const ACTIVE_SHEET_SHARE_DATA: "SHARE_DATA" = "SHARE_DATA"
export const ACTIVE_SHEET_SIGN_IN: "SIGN_IN" = "SIGN_IN"
export const ACTIVE_SHEET_SHARE_DATA_IN_FILE: "SHARE_DATA_IN_FILE" = "SHARE_DATA_IN_FILE"
...
export default function reducer(state: State, action: Action): State {
  ...
  if (
    ...
    const newValue = typeof action.payload === "undefined" ? !state[action.type] : action.payload

    if (state[action.type] === newValue) {
      // value hasn't changed
      return state
    } else {
      return { ...state, [action.type]: newValue }
    }
    ...
    return { ...state, activeSheet: ACTIVE_SHEET_SHARE_DATA_IN_FILE, exportFile: action.payload.file }
  } else if (action.type === "SET_SERVER_PORTS") {
    return { ...state, serverPorts: action.payload }
  } else if (action.type === "LAUNCHER_MSG") {
    return { ...state, launcherMsg: action.payload }
  } else {
    return state
  }
}
export const toggleAction = (type: BooleanActionType, payload?: boolean): Action => ({
  type,
  payload
})
