...
import { Button, colors, Link, Input, FlexColumn, FlexRow, FlexCenter, Textarea, Text, Glyph, styled } from "flipper"
...
type Props = {
  ...
  activePlugin: Class<FlipperPlugin<> | FlipperDevicePlugin<>> | null | undefined,
  onHide: () => any
}
    ...
    if (success) {
      content = (
        <FlexCenter grow={true}>
          <FlexColumn>
            <Center>
              <Glyph name="checkmark-circle" size={24} variant="outline" color={colors.light30} />
              <br />
          ...
          {this.props.activePlugin?.bugs && (
            <InfoBox>
              ...
              <span>
                ...
                <strong>{this.props.activePlugin?.title || this.props.activePlugin?.id} plugin</strong>
                {this.props.activePlugin?.bugs?.url && (
                  <span>
                    , you might find useful information about it here:{" "}
                    <Link href={this.props.activePlugin?.bugs?.url || ""}>{this.props.activePlugin?.bugs?.url}</Link>
                  </span>
                )}
                {this.props.activePlugin?.bugs?.email && (
                  <span>
                    , you might also want contact{" "}
                    <Link href={"mailto:" + String(this.props.activePlugin?.bugs?.email)}>
                      {this.props.activePlugin?.bugs?.email}
                    </Link>
...
export default connect(({ plugins: { devicePlugins, clientPlugins }, connections: { selectedPlugin } }) => ({
