...
> {
  ...
  render() {
    const { entry, status, version, description, managed, name, rootDir } = this.props.plugin
    return (
      <Plugin>
        <TitleRow>
          <PluginGlyph name="apps" size={24} variant="outline" color={colors.light30} />
          <Title>{name}</Title>
          &nbsp;
        ...
        <FlexRow>
          {managed ? (
            ...
            <ButtonGroup>
              {status === "outdated" && (
                <Button disabled={this.state.working} onClick={() => this.npmAction("update")}>
                  Update
                </Button>
              )}
              {INSTALLED.includes(status) ? (
                ...
                <Button disabled={this.state.working} onClick={() => this.npmAction("install")}>
                  Install
                </Button>
              )}
          ...
          {plugins
            ...
            .map((plugin: PluginT) => (
              <PluginItem
                plugin={plugin}
                key={plugin.name}
                onChangeState={action => this.onChangePluginState(plugin.name, action)}
              />
            ))}
