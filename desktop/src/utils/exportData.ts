...
import { PluginStatesState as PluginStates } from "../reducers/pluginStates"
...
export async function getStoreExport(
  ...
  const pluginsMap: Map<string, Class<FlipperDevicePlugin<> | FlipperPlugin<>>> = new Map([])
  ...
  for (let client of clients) {
    for (let plugin of client.plugins) {
      const pluginClass: Class<FlipperDevicePlugin<> | FlipperPlugin<>> | null | undefined = plugin
