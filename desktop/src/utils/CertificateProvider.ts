...
const adb = require("adbkit-fb") // Desktop file paths
...
export default class CertificateProvider {
  ...
  ensureOpenSSLIsAvailable(): undefined {
    if (!opensslInstalled()) {
  ...
  deployFileToMobileApp(
    ...
    if (os === "Android") {
      ...
      return Promise.all([deviceIdPromise, appNamePromise]).then(([deviceId, appName]) =>
        this.pushFileToAndroidDevice(deviceId, appName, destination + filename, contents)
      )
    }

    if (os === "iOS" || os === "windows") {
      return promisify(fs.writeFile)(destination + filename, contents).catch(err => {
        if (os === "iOS") {
      ...
      .then((devices: Array<{ id: string }>) => {
        const deviceMatchList = devices.map(device =>
          this.androidDeviceHasMatchingCSR(deviceCsrFilePath, device.id, appName, csr)
            ...
            .catch(e => {
              console.error(`Unable to check for matching CSR in ${device.id}:${appName}`, logTag)
              return {
                id: device.id,
                isMatch: false
              }
  ...
  androidDeviceHasMatchingCSR(directory: string, deviceId: string, processName: string, csr: string): Promise<boolean> {
    return this.executeCommandOnAndroid(deviceId, processName, `cat ${directory + csrFileName}`)
      .then(deviceCsr => {
  ...
  iOSDeviceHasMatchingCSR(directory: string, deviceId: string, bundleId: string, csr: string): Promise<boolean> {
    const originalFile = this.getRelativePathInAppContainer(path.resolve(directory, csrFileName))
    return tmpDir({
      unsafeCleanup: true
    })
      .then(dir => {
        return iosUtil.pull(deviceId, originalFile, bundleId, dir).then(() => dir)
      })
      .then(dir => {
        return promisify(fs.readdir)(dir)
          ...
          .then(fileName => {
            ...
            return promisify(fs.readFile)(copiedFile).then(data => this.santitizeString(data.toString()))
          })
      })
  ...
  pushFileToAndroidDevice(deviceId: string, app: string, filename: string, contents: string): Promise<void> {
  ...
  executeCommandOnAndroid(deviceId: string, user: string, command: string): Promise<string> {
  ...
  ensureCertificateAuthorityExists(): Promise<void> {
    ...
    return this.checkCertIsValid(caCert).catch(e => this.generateCertificateAuthority())
  }
  ...
  writeToTempFile(content: string): Promise<string> {
    return tmpFile().then((path, fd, cleanupCallback) => promisify(fs.writeFile)(path, content).then(_ => path))
  }
}
