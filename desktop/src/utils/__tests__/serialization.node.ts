...
class TestObject extends Object {
  constructor(title: Object, map: Map<any, any> | null | undefined, set: Set<any> | null | undefined) {
    super()
    this.title = title
    this.map = map
    this.set = set
  ...
  title: Object
  map: Map<any, any> | null | undefined
  set: Set<any> | null | undefined
}
...
test("test makeObjectSerializable function for unnested object with no Set and Map", () => {
  ...
  expect(output).toEqual(obj) // Testing numbers
...
test("test makeObjectSerializable and deserializeObject function for custom Object", () => {
  ...
  expect(output).toEqual(obj)
  expect(deserializeObject(output)).toEqual(obj)
  const nestedObj = new TestObject({
    title: "nestedTitle"
  })
  ...
  expect(deserializeObject(nestedObjWithMapSetOutput)).toEqual(nestedObjWithMapSet)
})
...
test("test serialize and deserializeObject function for non Object input", () => {
  ...
  expect(deserializeObject(makeObjectSerializable("octopus"))).toEqual("octopus")
...
test("test makeObjectSerializable and deserializeObject function for Map of Sets", () => {
  const map = new Map([["k1", new Set([1, 2, 3, 4, 5, 6])], [new Set([1, 2]), new Map([["k3", "v3"]])]])
