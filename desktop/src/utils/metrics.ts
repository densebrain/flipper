...
export function reportPlatformFailures<T>(promise: Promise<T>, name: string): Promise<T> {
  return promise.then(
    fulfilledValue => {
...
export function reportPluginFailures<T>(promise: Promise<T>, name: string, plugin: string): Promise<T> {
  return promise.then(
    fulfilledValue => {
...
export function tryCatchReportPlatformFailures<T>(closure: () => T, name: string): T {
  try {
