...
export function getDistanceRect(a: Rect, b: Rect): number {
  ...
  let xDifference = mostLeft.left === mostRight.left ? 0 : mostRight.left - (mostLeft.left + mostLeft.width)
  xDifference = Math.max(0, xDifference)
  const upper = a.top < b.top ? a : b
  const lower = b.top < a.top ? a : b
  let yDifference = upper.top === lower.top ? 0 : lower.top - (upper.top + upper.height)
