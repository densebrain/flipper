...
import { Component, DataDescription, Text, Panel, ManagedDataInspector, colors, styled } from "flipper"
    ...
    return (
      ...
      >
        ...
        <p>
          ...
          <span key="sep">: </span>
          <DataDescription type="number" value={event.endTime} setValue={function(path: Array<string>, val: any) {}} />
        </p>
        <p>
          <DataDescriptionKey>Source</DataDescriptionKey>
          <span key="sep">: </span>
          <DataDescription type="string" value={event.source} setValue={function(path: Array<string>, val: any) {}} />
        </p>
        <p>
          <DataDescriptionKey>Requested on cold start</DataDescriptionKey>
          <span key="sep">: </span>
          <DataDescription type="boolean" value={event.coldStart} setValue={null} />
        </p>
...
class RequestHeader extends Component<{
  ...
  render() {
    ...
    return (
      <Text>
        {event.viewport ? "Request" : "Prefetch"} at {this.dateString(event.startTime)} ({durationMs}ms)
      </Text>
    )
  }
}
