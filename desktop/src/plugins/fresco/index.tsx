...
import { FlipperPlugin, FlexRow, Text, DetailSidebar, colors, styled } from "flipper"
...
export default class extends FlipperPlugin<PluginState, *, PersistedState> {
  ...
  init() {
    ...
    this.client.subscribe("debug_overlay_event", (event: FrescoDebugOverlayEvent) => {
      this.setState({
    ...
    const imageList = images.map((image: CacheInfo) => {
      const imageIdList = image.imageIds.filter(imageID => {
        const filteredEvents = events.filter((event: ImageEventWithId) => {
          const output =
            event.attribution && event.attribution.length > 0 && event.imageIds && event.imageIds.includes(imageID)
  ...
  updateImagesOnUI = (images: ImagesList, surface: string, coldStart: boolean) => {
    ...
    if (selectedImage == null) {
      return (
        <EmptySidebar grow={true}>
          <Text align="center">Select an image to see the events associated with it.</Text>
        </EmptySidebar>
      )
    }
