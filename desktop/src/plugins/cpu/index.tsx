...
import { FlexColumn, FlexRow, Button, Toolbar, Text, ManagedTable, colors, lighten } from 'flipper';
type ADBClient = any;
type AndroidDevice = {
  adb: ADBClient;
};
...
export default class CPUFrequencyTable extends FlipperDevicePlugin<CPUState> {
  adbClient: ADBClient;
  intervalID: IntervalID | null | undefined;
  state = {
    cpuFreq: [],
    cpuCount: 0,
    monitoring: false,
  ...
  executeShell = (callback: ShellCallBack, command: string) => {
    this.adbClient.shell(this.device.serial, command).then(adb.util.readAll).then(function (output) {
  ...
  buildRow = (freq: CPUFrequency) => {
    ...
    if (freq.scaling_cur_freq == -2) {
      style = {
        style: {
          backgroundColor: lighten(colors.backgroundSelected, 0.4),
          color: colors.text,
          fontWeight: 700
        }
      };
    ...
    return {
      columns: {
        cpu_id: {
          value: <Text>CPU_{freq.cpu_id}</Text>
        },
        scaling_cur_freq: {
