...
import { Request, Response, Header } from "./index"
import { Component, FlexColumn, ManagedTable, ManagedDataInspector, Text, Panel, Select, styled, colors } from "flipper"
import { getHeaderValue } from "./index"
import querystring from "querystring" // $FlowFixMe
...
function decodeBody(container: Request | Response): string {
  ...
  const body = getHeaderValue(container.headers, "Content-Encoding") === "gzip" ? decompress(b64Decoded) : b64Decoded // Data is transferred as base64 encoded bytes to support unicode characters,
    ...
    return [
      {
        columns: {
          key: {
            value: <WrappingText>Full URL</WrappingText>
          },
          value: {
      ...
      },
      {
        columns: {
          key: {
            value: <WrappingText>Host</WrappingText>
          },
          value: {
      ...
      },
      {
        columns: {
          key: {
            value: <WrappingText>Path</WrappingText>
          },
          value: {
      ...
      },
      {
        columns: {
          key: {
            value: <WrappingText>Query String</WrappingText>
          },
          value: {
...
class QueryInspector extends Component<{
...
class XMLText extends Component<{
  body: any
}> {
  static NoScrollbarText = styled(Text)({
    overflowY: "hidden"
...
class JSONTextFormatter {
  formatRequest = (request: Request) => {
    return this.format(decodeBody(request), getHeaderValue(request.headers, "content-type"))
  }
  formatResponse = (request: Request, response: Response) => {
    return this.format(decodeBody(response), getHeaderValue(response.headers, "content-type"))
  }
...
class JSONFormatter {
  ...
  format = (body: string, contentType: string) => {
    if (
      ...
      try {
        const data = JSON.parse(body)
        return <ManagedDataInspector collapsed={true} expandRoot={true} data={data} />
      } catch (SyntaxError) {
        // Multiple top level JSON roots, map them one by one
        const roots = body.split("\n")
        return <ManagedDataInspector collapsed={true} expandRoot={true} data={roots.map(json => JSON.parse(json))} />
      }
    }
  }
}
...
class FormUrlencodedFormatter {
  formatRequest = (request: Request) => {
    ...
    if (contentType.startsWith("application/x-www-form-urlencoded")) {
      return <ManagedDataInspector expandRoot={true} data={querystring.parse(decodeBody(request))} />
    }
  }
}
