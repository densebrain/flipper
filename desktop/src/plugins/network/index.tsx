...
export default class extends FlipperPlugin<State, *, PersistedState> {
  ...
  static persistedStateReducer = (
    ...
    const dataType: "requests" | "responses" = data.url ? "requests" : "responses"
    return { ...persistedState, [dataType]: { ...persistedState[dataType], [data.id]: data } }
  }
  static getActiveNotifications = (persistedState: PersistedState): Array<Notification> => {
    const responses = persistedState ? persistedState.responses || [] : [] // $FlowFixMe Object.values returns Array<mixed>, but we know it is Array<Response>
    ...
    return r // Show error messages for all status codes indicating a client or server error
      ...
      .map((response: Response) => ({
        ...
        message: `Request to "${persistedState.requests[response.id]?.url || "(URL missing)"}" failed. ${
          response.reason
        }`,
        severity: "error",
  ...
  if (Object.keys(nextProps.requests).length === 0) {
    ...
    for (const responseId in nextProps.responses) {
      if (props.responses[responseId] == null) {
        ...
        const index = rows.findIndex(r => r.key === nextProps.requests[responseId]?.id)
