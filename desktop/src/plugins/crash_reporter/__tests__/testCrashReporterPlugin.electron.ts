...
import { PersistedState, Crash } from "../../crash_reporter"
import {
  parseCrashLog,
...
import { getPluginKey, getPersistedState } from "../../../utils/pluginUtils"
...
test("test defaultPersistedState of CrashReporterPlugin", () => {
  expect(CrashReporterPlugin.defaultPersistedState).toEqual({
    crashes: []
  })
})
...
test("test getNewPersisitedStateFromCrashLog for non-empty defaultPersistedState and defined pluginState", () => {
  ...
  const newPersistedState = getNewPersisitedStateFromCrashLog(perisistedState, CrashReporterPlugin, content, "iOS")
...
test("test getNewPersisitedStateFromCrashLog when os is undefined", () => {
  ...
  const perisistedState = getPersistedState(pluginKey, CrashReporterPlugin, pluginStates)
  const content = "Blaa Blaaa \n Blaa Blaaa"
  const newPersistedState = getNewPersisitedStateFromCrashLog(perisistedState, CrashReporterPlugin, content)
