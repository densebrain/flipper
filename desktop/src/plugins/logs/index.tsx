 ...
 * @format
 */
import { TableBodyRow, TableColumnOrder, TableColumnSizes, TableColumns, TableBodyColumn, ThemedProps, ThemedClassNames, ThemedClassMap, ThemedClassesProps, FlipperPluginProps, LogLevel, Theme, Filter, ThemeStylesCallback } from 'flipper';
import { Counter } from './LogWatcher';
import { DeviceLogEntry } from '../../devices/BaseDevice';
import { PluginComponentProps as PluginProps } from '../../plugin';
import _ from 'lodash';
import { getValue } from 'typeguard';
import { jss, Text, ManagedTable, Button, colors, ContextMenu, FlexColumn, Glyph, DetailSidebar, FlipperDevicePlugin, SearchableTable, styled, Device, createPaste, textContent, styleCreator, withStyles, ManagedTableDataPage } from 'flipper';
...
type Classes = "logRowUnknown" | "logRowVerbose" | "logRowDebug" | "logRowInfo" | "logRowWarn" | "logRowError" | "logRowFatal";
type LogLevelConfig = {
...
function makeLogLevelConfig(level: LogLevel, label: string = _.capitalize(level), icon?: string | null | undefined = null): LogLevelConfig {
  return {
    level,
    label,
    icon,
...
} = [['unknown'], ['verbose'], ['debug'], ['info', 'Info', 'info-circle'], ['warn', 'Warn', 'caution-triangle'], ['error', 'Error', 'caution-octagon'], ['fatal', 'Fatal', 'stop']].map(args => makeLogLevelConfig(...args)).reduce((map, config) => {
...
const baseStyles: ThemeStylesCallback<Classes> = (theme: Theme) => {
  ...
  return { ...Object.keys(LogLevelConfigs).reduce((map, level) => {
      // noinspection BadExpressionStatementJS
      ((level as any) as LogLevel);
      const config = LogLevelConfigs[level],
            logStyle = logs[level],
            iconBg = colors[(level as any)] || logStyle.color,
            iconText = getContrastText(iconBg);
      map[config.classNameAlias] = { ...logStyle,
        '& .logIcon, & .logCount': {
...
class LogTable extends FlipperDevicePlugin<State, Actions, PersistedState, {}> {
  initTimer: TimeoutID | null | undefined;
  batchTimer: TimeoutID | null | undefined;
  ...
  restoreSavedCounters = (): Array<Counter> => {
    ...
    return JSON.parse(savedCounters).map((counter: Counter) => ({ ...counter,
  ...
  calculateHighlightedRows = (deepLinkPayload: string | null | undefined, pages: Array<LogRecordsPage>): Set<string> => {
    ...
    for (let page of pagesReversed) {
      ...
      for (let i = page.items.length - 1; i >= 0; i--) {
        ...
        if (item.filterValue && item.filterValue.includes(deepLinkPayload)) {
  ...
  processEntry(entry: DeviceLogEntry, key: string): LogRecord {
    const config = LogLevelConfigs[(entry.type as any)]; //const {icon, style} = LOG_TYPES(theme)[(entry.type: string)] || LOG_TYPES(theme).debug;
        ...
        columns: {
          type: {
            ...
            render: (col: TableBodyColumn) => {
              const {
                icon
              } = LogLevelConfigs[col.value.type];
              return col.value.count > 1 ? <LogCount className="logCount">{col.value.count}</LogCount> : icon ? <Icon name={icon} className="logIcon" /> : null;
            }
          },
          time: {
  ...
  componentWillReceiveProps(nextProps: FlipperPluginProps<PersistedState, {}>, nextContext: any): undefined {
  ...
  componentDidUpdate(prevProps: FlipperPluginProps<PersistedState, {}>, prevState: State, snapshot: any): undefined {
    const {
  ...
  clearLogs = () => {
    ...
    this.setState({
      ...
      pages: [],
      highlightedRows: new Set(),
      counters: this.state.counters.map(counter => ({ ...counter,
        count: 0
      }))
    });
  };
  createPaste = () => {
    ...
    const mapFn = ({
      row
    }): string => Object.keys(COLUMNS).map(key => textContent(row.columns[key].value)).join('\t');
  ...
  render() {
    ...
    return <LogTable.ContextMenu buildItems={this.buildContextMenuItems} component={FlexColumn}>
        <SearchableTable innerRef={this.setTableRef} floating={false} multiline={true} columnSizes={this.columnSizes} columnOrder={this.columnOrder} columns={this.columns} rowType='page' items={this.state.pages} highlightedRows={this.state.highlightedRows} onRowHighlighted={this.onRowHighlighted} multiHighlight={true} rowGetter={this.rowGetter} defaultFilters={DEFAULT_FILTERS} zebra={false} actions={<Button onClick={this.clearLogs}>Clear Logs</Button>} // If the logs is opened through deeplink, then don't scroll as the row is highlighted
