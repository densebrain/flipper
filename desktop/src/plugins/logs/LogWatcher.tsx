...
import { PureComponent, FlexColumn, Panel, Input, Toolbar, Text, ManagedTable, Button, colors, styled } from "flipper"
export type Counter = {
  expression: RegExp,
  count: number,
  notify: boolean,
...
const Count = styled(Text)(({ theme: { colors } }) => ({
  filterProps: ["floating", "padded", "collapsed"],
  alignSelf: "center",
...
export default class LogWatcher extends PureComponent<Props, State> {
  ...
  onAdd = () => {
    if (
      this.props.counters.findIndex(({ label }) => label === this.state.input) > -1 ||
  ...
  resetCount = (index: number) => {
    ...
    newCounters[index] = { ...newCounters[index], count: 0 }
  ...
  buildRows = (): Array<TableBodyRow> => {
    return this.props.counters.map(({ label, count, notify }, i) => ({
      columns: {
        ...
        notify: {
          value: <Checkbox type="checkbox" checked={notify} onChange={() => this.setNotification(i, !notify)} />
        }
      },
      key: label
    }))
  ...
  setNotification = (index: number, notify: boolean) => {
    ...
    newCounters[index] = { ...newCounters[index], notify }
    this.props.onChange(newCounters)
  }
  ...
  onKeyDown = (e: SyntheticKeyboardEvent<>) => {
    if ((e.key === "Delete" || e.key === "Backspace") && this.state.highlightedRow != null) {
      this.props.onChange(this.props.counters.filter(({ label }) => label !== this.state.highlightedRow))
    }
  }
  ...
  render() {
    return (
      <FlexColumn grow={true} tabIndex={-1} onKeyDown={this.onKeyDown}>
        <WatcherPanel heading="Expression Watcher" floating={false} padded={false}>
          <Toolbar>
            <ExpressionInput
            ...
            />
            <Button onClick={this.onAdd} disabled={this.state.input.length === 0}>
              Add counter
            </Button>
          </Toolbar>
          <ManagedTable
