...
const Metro = require(`${DESKTOP_DIR}/static/node_modules/metro`);
...
function startMetroServer(app) {
  ...
  return Metro.runMetro({
    ...
    transformer: {
      babelTransformerPath: path.join(STATIC_DIR, 'transforms', 'index.js'),
    },
    resolver: {
    ...
    app.use((req,res,next) => {
      ...
      // Fix source map requests for Jetbrains products
      const local3k = /^\/localhost:3000/;
      req.url = req.url.replace(local3k, "");
      
      if (req.url === "/src/init.js") {
        req = {
          ...req,
          url: "/src/init.bundle"
        };
      }
      metroBundlerServer.processRequest(req,res,next)// .bind(metroBundlerServer)
    });
  });
}
...
function addWebsocket(server) {
  ...
  fs.watch(path.join(DESKTOP_DIR, 'src'), () => {
    io.emit('refresh');
  });
...
(async () => {
  ...
  outputScreen(socket);
  launchElectron({
    bundleURL: `http://localhost:${port}/src/init.js`,
