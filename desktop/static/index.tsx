...
ipcMain.on("sendNotification", (e, { payload, pluginNotification, closeAfter }) => {
  // notifications can only be sent when app is ready
  if (appReady) {
    const n = new Notification(payload) // Forwarding notification events to renderer process
...
function tryCreateWindow() {
  if (appReady && pluginsCompiled) {
    ...
    win.once("close", ({ sender }) => {
      ...
      const [width, height] = sender.getSize() // save window position and size
