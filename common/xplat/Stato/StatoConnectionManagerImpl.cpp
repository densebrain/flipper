...
#include "StatoRSocketResponder.h"
...
class ConnectionEvents : public rsocket::RSocketConnectionEvents {
 private:
  StatoConnectionManagerImpl* websocket_;

 public:
  ConnectionEvents(StatoConnectionManagerImpl* websocket)
...
StatoConnectionManagerImpl::StatoConnectionManagerImpl(
    StatoInitConfig config,
...
void StatoConnectionManagerImpl::start() {
...
void StatoConnectionManagerImpl::startSync() {
...
void StatoConnectionManagerImpl::doCertificateExchange() {
...
void StatoConnectionManagerImpl::connectSecurely() {
  rsocket::SetupParameters parameters;
  folly::SocketAddress address;
...
void StatoConnectionManagerImpl::reconnect() {
  folly::makeFuture()
      .via(statoEventBase_->getEventBase())
...
void StatoConnectionManagerImpl::stop() {
  if (client_) {
    client_->disconnect();
  }
  client_ = nullptr;
...
bool StatoConnectionManagerImpl::isOpen() const {
  return isOpen_ && connectionIsTrusted_;
}

void StatoConnectionManagerImpl::setCallbacks(Callbacks* callbacks) {
  callbacks_ = callbacks;
}

void StatoConnectionManagerImpl::sendMessage(const folly::dynamic& message) {
...
void StatoConnectionManagerImpl::onMessageReceived(
    const folly::dynamic& message,
    std::unique_ptr<StatoResponder> responder) {
  callbacks_->onMessageReceived(message, std::move(responder));
}

bool StatoConnectionManagerImpl::isCertificateExchangeNeeded() {
...
void StatoConnectionManagerImpl::requestSignedCertFromStato() {
...
void StatoConnectionManagerImpl::sendLegacyCertificateRequest(
    folly::dynamic message) {
...
bool StatoConnectionManagerImpl::isRunningInOwnThread() {
