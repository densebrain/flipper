...
using namespace facebook::stato::logger;
...
bool ConnectionContextStore::resetStatoDir() {
  ...
  if (stat(dirPath.c_str(), &info) != 0) {
    ...
    log("ERROR: Stato path exists but is not a directory: " + dirPath);
    return false;
  }
}
...
std::string loadStringFromFile(std::string fileName) {
  if (!fileExists(fileName)) {
    log("WARN: Stato path does not exist: " + fileName);
    return "";
  }
  ...
  debugStream() << "DEBUG: Stato content: " << s;
  return s;
}
