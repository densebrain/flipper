...
class StatoPlugin {
  ...
  */
  ...
  and the corresponding plugin on the Stato desktop app. The provided
  connection can be used to register method receivers as well as send
  messages back to the desktop app.
  */
  virtual void didConnect(std::shared_ptr<StatoConnection> conn) = 0;

  /**
  Called when a plugin has been disconnected and the StatoConnection
