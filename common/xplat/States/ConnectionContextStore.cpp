...
using namespace facebook::states::logger;

static constexpr auto CSR_FILE_NAME = "app.csr";
static constexpr auto STATES_CA_FILE_NAME = "sonarCA.crt";
...
bool ConnectionContextStore::hasRequiredFiles() {
  std::string caCert =
      loadStringFromFile(absoluteFilePath(STATES_CA_FILE_NAME));
...
std::shared_ptr<SSLContext> ConnectionContextStore::getSSLContext() {
  ...
  sslContext->loadTrustedCertificates(
      absoluteFilePath(STATES_CA_FILE_NAME).c_str());
...
bool ConnectionContextStore::resetStatesDir() {
  ...
  if (stat(dirPath.c_str(), &info) != 0) {
    ...
    log("ERROR: States path exists but is not a directory: " + dirPath);
    return false;
  }
}
...
std::string loadStringFromFile(std::string fileName) {
  if (!fileExists(fileName)) {
    log("WARN: States path does not exist: " + fileName);
    return "";
  }
  ...
  debugStream() << "DEBUG: States content: " << s;
  return s;
}
